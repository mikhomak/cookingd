...
impl LoginQuery {
    ...
    async fn verify_token<'a>(&self, ctx: &'a Context<'_>, token : String) -> FieldResult<LoginInfo> {
            ...
            Ok(pool) => {
                ...
                match r_token {
                    Ok(tokenData)=>{
                        ...
                        match r_user_model {
                            Ok(user_model) => {
                                Ok(LoginInfo{token, user: UserModel::convert_to_gql(&user_model) })
                            }
                            Err(error) => {
                                error!("Cannot create a token for the user with id {} due to error {}", email.clone(), error.message);
                                Err(async_graphql::Error::new("[LOGIN_003] Cannot find a user from token!")
